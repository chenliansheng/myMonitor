...
def monitorRedisAvailable():
    redisHost = 'localhost'
    redisPort = 63790
    msg = 'Redis连接失败: host=' + redisHost+':'+str(redisPort)
    try:
        r = redis.Redis(host=redisHost, port=redisPort)
        r.set("mymonitortestkey---------", "test")
        r.get("mymonitortestkey---------")
    except Exception as e:
        msg = 'Redis连接失败: host='+redisHost
